...
export default function ProductsPage() {
  ...
  const [formData, setFormData] = useState({
    name: "", description: "", price: "", discount: "0", stock: "", subcategories: [] as string[], flavors: [] as string[]
  });
  ...
  const openAddModal = () => {
    ...
    setFormData({ name: "", description: "", price: "", discount: "0", stock: "", subcategories: [], flavors: [] });
    setImageFile(null);
    setImagePreview("");
    setShowModal(true);
  };
      ...
      {showModal && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(15, 23, 42, 0.6)', backdropFilter: 'blur(4px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9999, padding: '1rem' }} onClick={() => setShowModal(false)}>
          <div onClick={e => e.stopPropagation()} style={{ background: 'white', borderRadius: '20px', width: '100%', maxWidth: '700px', maxHeight: '90vh', boxShadow: '0 25px 50px -12px rgba(0, 0, 0, 0.25)', overflow: 'hidden', display: 'flex', flexDirection: 'column', animation: 'popIn 0.3s ease' }}>
            ...
            <div style={{ background: 'linear-gradient(135deg, #d946a6 0%, #be185d 100%)', padding: '1.5rem', color: 'white', flexShrink: 0 }}>
            ...
            <form onSubmit={handleSubmit} style={{ padding: '1.5rem', overflowY: 'auto', flex: 1 }}>
              <div className="row">
                <div className="col-md-8">
                  <div style={{ marginBottom: '1rem' }}>
                    ...
                    <input type="text" value={formData.name} onChange={(e) => setFormData({...formData, name: e.target.value})} required placeholder="Nom du produit" style={{ width: '100%', padding: '0.75rem', border: '2px solid #e2e8f0', borderRadius: '10px', outline: 'none' }} onFocus={(e) => e.target.style.borderColor = '#d946a6'} onBlur={(e) => e.target.style.borderColor = '#e2e8f0'} />
                  </div>
                </div>
              </div>
              ...
              <div style={{ marginBottom: '1rem' }}>
                ...
                <textarea value={formData.description} onChange={(e) => setFormData({...formData, description: e.target.value})} rows={3} placeholder="Description du produit" style={{ width: '100%', padding: '0.75rem', border: '2px solid #e2e8f0', borderRadius: '10px', outline: 'none', resize: 'none' }} onFocus={(e) => e.target.style.borderColor = '#d946a6'} onBlur={(e) => e.target.style.borderColor = '#e2e8f0'} />
              </div>
                ...
                <div style={{ border: '2px solid #e2e8f0', borderRadius: '10px', padding: '0.75rem', maxHeight: '150px', overflowY: 'auto' }}>
                  {groupedSubcategories.map((group: any) => (
                    <div key={group.category._id} style={{ marginBottom: '0.5rem' }}>
                      <div style={{ fontWeight: 600, color: '#d946a6', fontSize: '0.85rem', marginBottom: '0.25rem' }}>{group.category.name}</div>
                      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.5rem', marginLeft: '0.5rem' }}>
                        {group.subcats.map((sub: any) => (
                          <label key={sub._id} style={{ display: 'flex', alignItems: 'center', gap: '0.25rem', padding: '0.25rem 0.5rem', background: formData.subcategories.includes(sub._id) ? '#fef2f2' : '#f8fafc', border: formData.subcategories.includes(sub._id) ? '1px solid #d946a6' : '1px solid #e2e8f0', borderRadius: '6px', cursor: 'pointer', fontSize: '0.85rem' }}>
              ...
              <div style={{ marginBottom: '1rem' }}>
                <label style={{ display: 'block', fontSize: '0.75rem', fontWeight: 600, color: '#64748b', textTransform: 'uppercase', marginBottom: '0.5rem' }}>
                  <i className="mdi mdi-palette me-1"></i>Goûts <span className="badge bg-info ms-2">{formData.flavors.length}</span>
                </label>
                <div style={{ border: '2px solid #e2e8f0', borderRadius: '10px', padding: '0.75rem', maxHeight: '120px', overflowY: 'auto' }}>
                  {flavors.length > 0 ? (
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.5rem' }}>
                      {flavors.map((flavor: any) => (
                        <label key={flavor._id} style={{ display: 'flex', alignItems: 'center', gap: '0.25rem', padding: '0.25rem 0.5rem', background: formData.flavors.includes(flavor._id) ? '#ede9fe' : '#f8fafc', border: formData.flavors.includes(flavor._id) ? '1px solid #8b5cf6' : '1px solid #e2e8f0', borderRadius: '6px', cursor: 'pointer', fontSize: '0.85rem' }}>
                          <input type="checkbox" checked={formData.flavors.includes(flavor._id)} onChange={(e) => {
                            const newFlavors = e.target.checked ? [...formData.flavors, flavor._id] : formData.flavors.filter(id => id !== flavor._id);
                    ...
                    <p style={{ margin: 0, color: '#94a3b8', fontSize: '0.85rem' }}>Aucun goût disponible</p>
                  )}
                </div>
                <small style={{ color: '#94a3b8', fontSize: '0.75rem' }}>Sélectionnez les goûts disponibles (optionnel)</small>
              </div>
