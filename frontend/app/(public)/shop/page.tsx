...
export default function ShopPage() {
  ...
  useEffect(() => {
    ...
    fetch('http://localhost:5000/api/flavors').then(r => r.json()).then(setFlavors);
        ...
        <div className="container" style={{ position: 'relative', zIndex: 1 }}>
          <nav aria-label="breadcrumb" style={{ marginBottom: '15px' }}>
            <ol className="breadcrumb" style={{ backgroundColor: 'transparent', padding: 0, margin: 0, display: 'flex', alignItems: 'center', gap: '8px' }}>
              ...
              <li className="breadcrumb-item active" style={{ color: 'white', fontSize: '13px', fontWeight: '600' }}>Boutique</li>
            </ol>
          </nav>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px', flexWrap: 'wrap' }}>
            ...
            <span style={{ background: 'rgba(255,255,255,0.25)', color: 'white', padding: '6px 14px', borderRadius: '20px', fontSize: '13px', fontWeight: '600', display: 'flex', alignItems: 'center', gap: '6px' }}>
              ...
              {products.filter(p => (p.discount ?? 0) > 0).length} en promo
            </span>
          </div>
        </div>
      </div>
        ...
        <div className="d-lg-none" style={{ marginBottom: '20px' }}>
          ...
          >
            <span style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
              ...
              Filtres et tri
              {(selectedBrand || selectedCategory || selectedFlavor || showDiscountOnly || showInStockOnly || priceRange.min || priceRange.max) && (
          ...
          {showMobileFilters && (
            <div style={{ marginTop: '15px', backgroundColor: 'white', borderRadius: '16px', padding: '20px', boxShadow: '0 4px 20px rgba(0,0,0,0.08)' }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '15px' }}>
                <select style={{ width: '100%', padding: '12px', border: '2px solid #fce7f3', borderRadius: '10px', fontSize: '14px', outline: 'none' }} value={selectedCategory} onChange={(e) => setSelectedCategory(e.target.value)}>
                  <option value="">Catégorie</option>
                  {categories.map((cat: any) => (<option key={cat._id} value={cat._id}>{cat.name}</option>))}
                </select>
                <select style={{ width: '100%', padding: '12px', border: '2px solid #fce7f3', borderRadius: '10px', fontSize: '14px', outline: 'none' }} value={selectedBrand} onChange={(e) => setSelectedBrand(e.target.value)}>
              ...
              <div style={{ display: 'flex', gap: '10px', marginBottom: '15px' }}>
                <label style={{ flex: 1, display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer', padding: '10px', background: showDiscountOnly ? '#fce7f3' : '#f8f9fa', borderRadius: '10px', fontSize: '13px', fontWeight: '500' }}>
                  <input type="checkbox" checked={showDiscountOnly} onChange={(e) => setShowDiscountOnly(e.target.checked)} style={{ accentColor: '#ec4899' }} />
                  <i className="fas fa-tag" style={{ color: '#ec4899' }}></i> Promos
                </label>
        ...
        <div className="row g-4">
          ...
          <div className="col-lg-3 d-none d-lg-block">
            <div style={{ backgroundColor: 'white', borderRadius: '16px', padding: '24px', boxShadow: '0 4px 20px rgba(236, 72, 153, 0.1)', border: '2px solid #fce7f3', position: 'sticky', top: '160px' }}>
              ...
              <div style={{ marginBottom: '20px' }}>
                <label style={{ display: 'flex', alignItems: 'center', gap: '10px', cursor: 'pointer', padding: '10px', background: showInStockOnly ? '#dcfce7' : '#fafafa', borderRadius: '10px', transition: 'all 0.3s', border: '2px solid', borderColor: showInStockOnly ? '#16a34a' : '#fce7f3' }}>
                  <input type="checkbox" checked={showInStockOnly} onChange={(e) => setShowInStockOnly(e.target.checked)} style={{ width: '16px', height: '16px', cursor: 'pointer', accentColor: '#16a34a' }} />
                  <span style={{ fontSize: '13px', color: showInStockOnly ? '#16a34a' : '#666', fontWeight: '600' }}>
                    <i className="fas fa-check-circle me-2"></i>En stock
                  </span>
                </label>
              </div>
